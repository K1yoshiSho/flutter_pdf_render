...
class PdfViewerState extends State<PdfImageViewer> with SingleTickerProviderStateMixin {
  ...
  void _determinePagesToShow() {
    ...
    final r = m.row0[0];
    final exposed =
        Rect.fromLTWH(-m.row0[3], -m.row1[3], _lastViewSize!.width, _lastViewSize!.height).inflate(_padding);
    ...
    for (final page in _pages!) {
      ...
      if (page.preview?.hasUpdatedTexture == true && fw <= page.preview!.texWidth! && fh <= page.preview!.texHeight!) {
        ...
        final tex = page._textures[page._textureId++ & 1] ??=
            await PdfPageImageTexture.create(pdfDocument: pdfPage.document, pageNumber: page.pageNumber);
